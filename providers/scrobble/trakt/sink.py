...
"""Trakt scrobble sink
- Retries (401 refresh, 429 backoff)
- Debounce non-start (STOP bypass at high progress)
- STOP->PAUSE under threshold + suspicious STOP jump demotion
- Backtrack-safe starts (respect large regressions)
- Progress memory (session/global); clamp only for PAUSE
- Prefer episode GUID IDs; fallback show+S/E; last-resort GUID search
- Debug-gated verbose logs (runtime.debug)
"""
...
def _cfg() -> dict:
    ...
    paths += [Path("/config/config.json"), Path("/app/config/config.json"), Path("./config.json"), Path("/mnt/data/config.json")]
    for p in paths:
        try:
...
def _app_meta(cfg: dict) -> dict:
    rt = (cfg.get("runtime") or {})
    av = rt.get("version") or APP_AGENT
    ad = (rt.get("build_date") or "").strip()
    m = {"app_version": str(av)}
    if ad: m["app_date"] = ad
    return m
...
def _get(path, cfg):  return requests.get(f"{TRAKT_API}{path}", headers=_hdr(cfg), timeout=10)
def _post(path, body, cfg): return requests.post(f"{TRAKT_API}{path}", headers=_hdr(cfg), json=body, timeout=10)
...
def _force_stop_at(cfg):  # default 95
...
def _log(msg: str, level: str = "INFO"):
    if BASE_LOG:
        try:
            ...
            pass
    if level.upper() == "DEBUG" and not _is_debug():
        return
    print(f"{level} [TRAKT] {msg}")

def _dbg(msg: str):
    if _is_debug():
        print(f"DEBUG [TRAKT] {msg}")
...
class TraktSink(ScrobbleSink):
    def __init__(self, logger=None):
        ...
        try:
            if self._logr and hasattr(self._logr,"set_level"):
                self._logr.set_level("INFO")
                if _is_debug():
                    self._logr.set_level("DEBUG")
        except Exception:
            pass
    ...
    def _send_http(self, path, body, cfg):
        ...
        if ev.action == "start":
            if p_now <= 2 and (p_sess >= 10 or p_glob >= 10):
                ...
                if p_now == 0 and p_glob > 0:
                    ...
                    p_send = p_glob         # small regression → keep memory
                else:
                    p_send = p_now          # big regression → trust now
        else:
            p_base = p_now
