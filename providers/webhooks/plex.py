# providers/webhooks/plex.py
# CrossWatch - Plex Scrobble Webhook Module
...
def _archive(event_type: str, media_type: str, md: dict[str, Any], ids: dict[str, Any], account: Any, prog: Any, **kw: Any) -> None:
    try:
        ...
        record_webhook(
            event_type=event_type, source_provider="plex", destination_provider="trakt",
            media_type=media_type, md=md, ids=ids, account=account, progress=prog, **kw,
        )
    except Exception:
        pass
...
def process_webhook(
    ...
    cfg = _ensure_scrobble(cfg if isinstance(cfg, dict) else _load_config())
    provider_instance = str(provider_instance or "default").strip() or "default"
    ...
    selected_rating_sinks = _sinks_from_webhook_cfg(cfg, provider_instance)
    ...
    watched_at = float(tset.get("watched_at", 90.0))
    ...
    if event == "media.rate":
        ...
        if enable_trakt_ratings:
            if rating_val == 0:
                ...
                if body_r:
                    ...
                    r = None
            if r is None:
                results["trakt"] = {"ok": False, "error": "no_ids"}
            else:
                try:
                    rj_r: Any = r.json()
                except Exception:
                    rj_r = {"raw": (r.text or "")[:200]}
                results["trakt"] = {"ok": r.status_code < 400, "status": r.status_code, "resp": rj_r}

        if (enable_simkl_ratings or enable_mdblist_ratings) and media_type in ("movie", "show"):
            try:
                from providers.scrobble.plex.watch import _mdblist_send_rating, _simkl_send_rating
            except Exception as e:
                sent = True
                if enable_simkl_ratings:
                    results["simkl"] = {"ok": False, "error": str(e)}
                if enable_mdblist_ratings:
                    results["mdblist"] = {"ok": False, "error": str(e)}
            else:
                if enable_simkl_ratings:
                    sent = True
        ...
        _archive("rating_failed", media_type, md, ids_all2, acc_title, None, feature="ratings", operation="rate", reason=str(results.get("status")))
        return results
    ...
    if intended == "/scrobble/stop" and not fast_cancel_stop:
        if event != "media.scrobble" and prog < force_stop_at and last_prog >= 0 and (prog - last_prog) >= 30 and last_prog < stop_pause_threshold and prog >= 98:
            _emit(logger, f"suppress suspicious final progress jump {last_prog:.0f}%→{prog:.0f}% (thr={force_stop_at})", "DEBUG")
            prog = last_prog
    ...
    sinks_cfg = _sinks_from_webhook_cfg(cfg, provider_instance)

    if media_type == "episode":
        dispatch_title = (md.get("grandparentTitle") or md.get("title") or "").strip()
        dispatch_season = md.get("parentIndex")
        dispatch_episode = md.get("index")
    else:
        dispatch_title = (md.get("title") or md.get("grandparentTitle") or "").strip()
        dispatch_season = None
        dispatch_episode = None
    _emit(logger, f"webhook intent {intended} using {_body_ids_desc(body) if body else _describe_ids(cw_ids)}, prog={prog}", "DEBUG")
    r = _dispatch_scrobble(
        "plex",
        intended,
        media_type=media_type,
        ids=cw_ids or ids_all2,
        title=dispatch_title,
        year=md.get("year"),
        season=dispatch_season,
        episode=dispatch_episode,
        progress=prog,
        account=acc_title,
