# providers/webhooks/jellyfin.py
# CrossWatch - Jellyfin Scrobbler Webhook Module
...
def _archive(event_type: str, media_type: str, md: dict[str, Any], payload: dict[str, Any], ids: dict[str, Any], account: Any, prog: Any, reason: str | None = None) -> None:
    try:
        ...
        if media_type == "episode":
            title = md.get("SeriesName") or _grab(payload, ["SeriesName", "SeriesTitle"])
            season, episode = _episode_numbers(md, payload)
        else:
            title, season, episode = md.get("Name"), None, None
        record_webhook(
...
def process_webhook(
    ...
    try:
        ...
        if intended == "/scrobble/stop":
            ...
            if last_prog >= 0 and (prog - last_prog) >= 30 and last_prog < stop_pause_threshold and prog >= 98:
                _emit(
                    logger,
        ...
        _emit(logger, f"webhook intent {intended} using {_body_ids_desc(body) if body else _ids_desc(cw_ids)}, prog={prog}", "DEBUG")
        r = _dispatch_scrobble(
            "jellyfin",
            intended,
            media_type=media_type,
            ids=cw_ids or ids_all,
            title=title,
            year=year,
            season=season_val,
            episode=episode_val,
