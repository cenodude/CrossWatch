...
class TraktSink(ScrobbleSink):
    ...
    def send(self, ev: ScrobbleEvent):
        ...
        action = ev.action
        if ev.action == "stop":
            # suspicious jump to near-end without ever crossing threshold
            if p_send >= 98 and last >= 0 and last < thr and (p_send - last) >= 30:
                self._log(f"Demote STOP→PAUSE (jump {last}%→{p_send}%, thr={thr})")
                action = "pause"
                p_send = last  # keep progress consistent with prior known value
            elif p_send < thr:
                action = "pause"

        # Update memory (now using the possibly-clamped p_send)
