...
def _server_find_item(server: Any, q: Mapping[str, Any], mtype: str) -> Optional[Any]:
    ...
    for key in ("imdb", "tmdb", "tvdb"):
        if ids.get(key):
            try:
                hits = server.search(guid=f"{key}://{ids[key]}", libtype=mtype) or []
...
class _PlexOPS:
    ...
    def add(self, cfg: Mapping[str, Any], items: Iterable[Mapping[str, Any]], *, feature: str, dry_run: bool=False) -> Dict[str, Any]:
        ...
        if feature == "watchlist":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _watchlist_add(acct, items)
            return {"ok": True, "count": cnt}

        if feature == "ratings":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _ratings_apply(env, items)
            return {"ok": True, "count": cnt}

        if feature == "history":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _history_apply(env, items, watched=True)
            return {"ok": True, "count": cnt}

        if feature == "playlists":
            if dry_run:
                return {"ok": True, "count": sum(len(it.get("items", [])) for it in items), "dry_run": True}
            applied = 0
            for pl in items:
    ...
    def remove(self, cfg: Mapping[str, Any], items: Iterable[Mapping[str, Any]], *, feature: str, dry_run: bool=False) -> Dict[str, Any]:
        ...
        if feature == "watchlist":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _watchlist_remove(acct, items)
            return {"ok": True, "count": cnt}

        if feature == "ratings":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _ratings_remove(env, items)
            return {"ok": True, "count": cnt}

        if feature == "history":
            if dry_run:
                return {"ok": True, "count": sum(1 for _ in items), "dry_run": True}
            cnt = _history_apply(env, items, watched=False)
            return {"ok": True, "count": cnt}
