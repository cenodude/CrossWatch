...
def _watchlist_path(adapter) -> Path:
...
def _load_state(adapter) -> Dict[str, Any]:
    """Load current watchlist state from disk."""
    p = _watchlist_path(adapter)
    try:
...
def _list_snapshots(adapter) -> List[Path]:
    ...
    if not d.exists() or not d.is_dir():
        return []
    return sorted(
        [p for p in d.iterdir() if p.is_file() and p.suffix == ".json" and p.name.endswith("-watchlist.json")],
