...
class Stats:
    ...
    def _build_union_map(state: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        """Return de-duplicated map: key -> {src, title, type} where src in {"plex","simkl","both"}."""
        plex = (state.get("plex", {}) or {}).get("items", {}) or {}
        simkl = (state.get("simkl", {}) or {}).get("items", {}) or {}

        # Build SIMKL buckets with alias mapping
        ...
        def ensure_bucket(d: Dict[str, Any]) -> str:
            # reuse bucket if any alias already mapped
            ...
            if pk in buckets:
                pk = f"{pk}#{len(buckets)}"
            buckets[pk] = {"src": "", "title": Stats._title_of(d), "type": (d.get("type") or "").lower()}
            for a in Stats._aliases(d):
                alias2bucket[a] = pk
            return pk
        ...
        for _, raw in simkl.items():
            bk = ensure_bucket(raw)
            buckets[bk]["src"] = "simkl"
            if not buckets[bk].get("title"):
                buckets[bk]["title"] = Stats._title_of(raw)
            if not buckets[bk].get("type"):
                buckets[bk]["type"] = (raw.get("type") or "").lower()

        # merge PLEX
        for _, raw in plex.items():
            # adopt SIMKL bucket if any alias matches
            bk = None
            for a in Stats._aliases(raw):
                if a in alias2bucket:
                    bk = alias2bucket[a]
                    break
            if bk is None:
                bk = ensure_bucket(raw)
            prev = buckets[bk].get("src") or ""
            buckets[bk]["src"] = "both" if prev and prev != "plex" else "plex"
            if not buckets[bk].get("title"):
                buckets[bk]["title"] = Stats._title_of(raw)
            if not buckets[bk].get("type"):
                buckets[bk]["type"] = (raw.get("type") or "").lower()
    ...
    def reset(self) -> None:
        ...
        with self.lock:
            ...
            self._save()  # sets generated_at and writes atomically
