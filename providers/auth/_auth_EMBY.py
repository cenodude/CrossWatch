...
class EmbyAuth(AuthProvider):
    ...
    def manifest(self) -> AuthManifest:
        return AuthManifest(
            ...
            flow="token",
            fields=[
                {"key": "emby.server",   "label": "Server URL", "type": "text",     "required": True},
                {"key": "emby.username", "label": "Username",   "type": "text",     "required": True},
                {"key": "emby.password", "label": "Password",   "type": "password", "required": True},
            ],
    ...
    def start(self, cfg: MutableMapping[str, Any], redirect_uri: str) -> Dict[str, Any]:
        ...
        try:
            me = requests.get(urljoin(base, "Users/Me"), headers=_headers(token, dev_id), timeout=10)
            if me.ok:
                info = me.json() or {}
