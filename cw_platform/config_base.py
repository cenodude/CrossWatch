...
def CONFIG_BASE() -> Path:
    ...
    Precedence:
      1) CONFIG_BASE env var
      2) /config if running in a container (/app exists)
...
DEFAULT_CFG: Dict[str, Any] = {
    ...
    # --- Batches & Pairs (UI driven) ----------------------------------------
    "batches": [
        { "name": "Default Batch", "enabled": True, "pair_ids": [], "id": "batch_default" }
    ],
    "pairs": [],

    # --- Optional UI/local knobs --------------------------------------------
    "ui": { "providers_overlay_style": "dark" },
}
...
def _deep_merge(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Non-destructive deep merge:
      ...
      - lists/other types are replaced by override
    """
...
def _normalize_features_map(f: dict | None) -> dict:
    """
    Accepts booleans or dicts for feature entries and normalizes to:
      { "enable": bool, "add": bool, "remove": bool }
    """
    f = dict(f or {})
...
def load_config() -> Dict[str, Any]:
    ...
    IMPORTANT: This function never writes to disk.
    """
    p = _cfg_file()
