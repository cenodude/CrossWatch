...
class StateStore:
    base_path: Path
    @property
    def state(self) -> Path: return self.base_path / "state.json"
    @property
    def tomb(self) -> Path: return self.base_path / "tombstones.json"
    @property
    def last(self) -> Path: return self.base_path / "last_sync.json"
    @property
    def hide(self) -> Path: return self.base_path / "watchlist_hide.json"
    @property
    def ratings_changes(self) -> Path: return self.base_path / "ratings_changes.json"
    ...
    def load_tomb(self) -> Dict[str, Any]:
        ...
        if "ttl_sec" not in t: t["ttl_sec"] = None
        return t
    ...
    def clear_watchlist_hide(self) -> None:
        try:
            if self.hide.exists(): self.hide.unlink()
        except Exception:
            try: self.hide.write_text("[]", encoding="utf-8")
            except Exception: pass
    def save_ratings_changes(self, data: Mapping[str, Any]) -> None:
        try: self._write_atomic(self.ratings_changes, data)
        except Exception: pass
