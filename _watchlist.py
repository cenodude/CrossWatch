...
def _type_from_item_or_guess(item: Dict[str, Any], key: str) -> str:
    ...
    if typ in {"movie","tv","show","series"}: return "movie" if typ == "movie" else "tv"
    ids = item.get("ids") or {}
    if ids.get("tvdb") or ids.get("thetvdb"): return "tv"
...
def _guid_variants_from_key_or_item(key: str, item: dict | None=None) -> list[str]:
    ...
    if not (prov and ident):
        ...
        elif ids.get("tvdb") or ids.get("thetvdb"): prov, ident = "tvdb", str(ids.get("tvdb") or ids.get("thetvdb"))
    if not (prov and ident): return []
...
def _jf_base(cfg: dict[str,Any]) -> str:
    ...
    if not base: raise RuntimeError("Jellyfin/Emby: missing 'server'")
...
def _jf_require_user(cfg: dict[str,Any]) -> str:
    ...
    if not uid: raise RuntimeError("Jellyfin/Emby: missing 'user_id'")
    return uid

def _extract_jf_id(item: dict, key: str) -> str | None:
    ids = (item or {}).get("ids") or {}
    cand = (ids.get("jellyfin") or ids.get("emby") or item.get("jellyfinId") or item.get("embyId") or
            item.get("jf_id") or item.get("Id") or item.get("id"))
    if cand: return str(cand)
    pref, _, val = (key or "").partition(":")
    return val.strip() if pref.lower().strip() in {"jellyfin","emby"} and val.strip() else None
...
def _jf_get(base: str, path: str, headers: dict[str,str], params: dict[str,Any]|None=None) -> dict[str,Any]:
    ...
    if not r.ok: raise RuntimeError(f"Jellyfin/Emby GET {path} -> {r.status_code}: {getattr(r,'text','')}")
    try: j = r.json()
    except Exception: j = {}
...
def build_watchlist(state: dict[str,Any], tmdb_ok: bool) -> list[dict[str,Any]]:
    ...
    bag = {p: (_get_items(state, p) or {}) for p in providers}
    all_keys = set().union(*(set(d.keys()) for d in bag.values() if isinstance(d, dict)))
    hidden, out = _load_hide_set(), []

    for key in all_keys:
        if key in hidden: continue
        candidates = [(p.lower(), bag[p].get(key) or {}) for p in providers if bag[p].get(key)]
        ...
        if "tv" in declared: typ = "tv"
            ...
            ids_ = (info.get("ids") or {}) | {k: info.get(k) for k in ("tmdb","imdb","tvdb","trakt","slug")}
        ...
        if epoch_map and any(epoch_map.values()):
            added_src = max(epoch_map, key=epoch_map.get); added_epoch = epoch_map[added_src]
        else:
...
def _delete_on_plex_single(key: str, state: dict[str,Any], cfg: dict[str,Any]) -> None:
    ...
    if str(rk or "").strip(): target_tokens.add(f"rk:{str(rk).strip()}")
    ...
    def matches(m) -> bool:
        ...
        if targets_guid:
            ...
            try:
                ...
                pass
            if any(c in targets_guid for c in cand_guids): return True
...
def _simkl_deleted_count(resp: dict[str,Any]) -> int:
    ...
    return sum(int(d.get(k,0) or 0) for k in ("movies","shows","episodes","seasons")) if isinstance(d,dict) else 0
...
def _delete_on_plex_batch(items: list[dict[str, Any]], state: dict[str,Any], cfg: dict[str,Any]) -> None:
    for it in items or []:
...
def delete_watchlist_batch(keys: list[str], prov: str, state: dict[str,Any], cfg: dict[str,Any]) -> dict[str,Any]:
    ...
    def _build_items(for_provider: str|None) -> list[dict]:
        arr: list[dict] = []
        for k in keys:
            it = (_find_item_in_state_for_provider(state, k, for_provider) if for_provider else _find_item_in_state(state, k)) or {}
    ...
    if prov == "ALL":
        ...
        if any(_del_key_from_provider_items(state, p, k) for p in _registry_sync_providers() for k in keys):
            _save_state_dict(_state_path(), state)
        return {"ok": ok_any, "deleted": len(keys), "provider": "ALL", "details": details}
...
def delete_watchlist_item(key: str, state_path: Path, cfg: dict[str,Any], log=None, provider: str|None=None) -> dict[str,Any]:
    ...
    def _present():
        return any(_get_provider_items(state, p).get(key) for p in _registry_sync_providers())

    def _delete_and_drop(p, fn):
        it = _find_item_in_state(state, key) or {}
        fn([{"key": key, "item": it, "type": _type_from_item_or_guess(it, key)}])
    ...
    try:
        if prov == "PLEX":
            ...
            _delete_and_drop("SIMKL", lambda items: _delete_on_simkl_batch(items, cfg.get("simkl", {}) or {}))
        elif prov == "TRAKT":
            _delete_and_drop("TRAKT", lambda items: _delete_on_trakt_batch(items, cfg.get("trakt", {}) or {}))
        elif prov == "JELLYFIN":
            ...
            return {"ok": any(v["ok"] for v in details.values()), "deleted": key, "provider": "ALL", "details": details}
        else:
