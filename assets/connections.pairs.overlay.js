...
/* ===== Pairs overlay (compact, matte, neon) ===== */
#cx-pairs-title{display:none!important;margin:0}
#cx-pairs .cx-conn{margin-top:8px!important}
#pairs_list{display:block}
.pairs-board{padding-bottom:12px;position:relative}

/* Card + permanent neon border */
.pair-card{
  position:relative;border-radius:16px;padding:12px 14px 10px;background:rgba(13,15,20,.92);overflow:hidden;
  box-shadow:0 0 0 1px rgba(255,255,255,.35),0 0 6px rgba(255,255,255,.25),0 8px 28px rgba(0,0,0,.32);
  transition:box-shadow .2s ease,transform .15s ease,filter .18s ease
}
.pair-card:hover{
  transform:translateY(-1px);
  box-shadow:0 0 0 1px #fff,0 0 10px rgba(255,255,255,.6),0 0 22px rgba(255,255,255,.45),0 0 38px rgba(255,255,255,.3),0 12px 36px rgba(0,0,0,.5)
}

/* Header */
.pair-head{display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:10px;line-height:1;position:relative;z-index:5}
.pair-card .pill{
  display:inline-block;padding:6px 12px;border-radius:999px;font-family:inherit;font-weight:800;font-size:.9rem;letter-spacing:.02em;color:#fff;
  background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.14);box-shadow:inset 0 0 0 1px rgba(255,255,255,.05),0 3px 10px rgba(0,0,0,.25)
}
.pair-card .arrow{color:#d7deee;opacity:.8;margin:0 2px}
.pair-card .mode{
  font-family:inherit;font-size:.84rem;font-weight:800;letter-spacing:.02em;color:#f0f3ff;margin-left:6px;padding:4px 10px;border-radius:999px;white-space:nowrap;
  background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);box-shadow:inset 0 0 0 1px rgba(255,255,255,.04)
}

/* Index badge */
.ord-badge{
  position:absolute;top:6px;left:12px;min-width:18px;height:18px;padding:0 6px;border-radius:999px;background:linear-gradient(135deg,#7b68ee,#a78bfa);
  color:#fff;font-size:11px;font-weight:850;line-height:18px;text-align:center;box-shadow:0 0 10px rgba(124,92,255,.45);z-index:6
}
...
.act-group{
  display:flex;gap:10px;padding:8px 10px;border-radius:14px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);
  box-shadow:inset 0 0 0 1px rgba(255,255,255,.04),0 6px 18px rgba(0,0,0,.22)
}
.icon-only{
  position:relative;display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border:0;border-radius:12px;cursor:pointer;color:#e5e8f2;
  background:#121224;border:1px solid rgba(255,255,255,.14);transition:transform .12s,background .12s,box-shadow .12s,color .12s
}
...
.icon-only:hover{background:#17172a;box-shadow:0 10px 24px rgba(0,0,0,.26);transform:translateY(-1px) scale(1.05)}
.icon-only:active{transform:none}
.icon-only input{position:absolute;inset:0;opacity:0;margin:0;cursor:pointer}
.icon-only input:checked + .ico{color:#fff;filter:drop-shadow(0 0 6px rgba(255,255,255,.85)) drop-shadow(0 0 14px rgba(255,255,255,.55));transform:scale(1.08)}
.icon-only.danger:hover{color:#ff5a5e}
@media (prefers-reduced-motion:reduce){.icon-only{transition:none!important}}

/* Brand imprints (subtle) */
.cx-conn{position:relative;overflow:hidden}
.cx-conn::before,.cx-conn::after{
  content:"";position:absolute;top:0;bottom:0;pointer-events:none;z-index:0;opacity:.12;background-repeat:no-repeat;background-position:center;background-size:170% auto
}
.cx-conn::before{left:0;width:52%;background-image:var(--src-logo);-webkit-mask-image:linear-gradient(90deg,#fff 0 44%,rgba(255,255,255,0) 66%);mask-image:linear-gradient(90deg,#fff 0 44%,rgba(255,255,255,0) 66%)}
.cx-conn::after{right:0;width:52%;background-image:var(--dst-logo);-webkit-mask-image:linear-gradient(-90deg,#fff 0 44%,rgba(255,255,255,0) 66%);mask-image:linear-gradient(-90deg,#fff 0 44%,rgba(255,255,255,0) 66%)}
.cx-conn .fusion{position:absolute;inset:0;pointer-events:none;z-index:2;border-radius:16px;background:linear-gradient(90deg,transparent 46%,rgba(255,255,255,.06) 50%,transparent 54%);opacity:.25}
.cx-conn.brand-plex::before{box-shadow:inset -220px 0 260px rgba(229,160,13,.18)}
.cx-conn.brand-simkl::before{box-shadow:inset -220px 0 260px rgba(0,183,235,.20)}
.cx-conn.brand-trakt::before{box-shadow:inset -220px 0 260px rgba(237,28,36,.18)}
.cx-conn.dst-plex::after{box-shadow:inset 220px 0 260px rgba(229,160,13,.18)}
.cx-conn.dst-simkl::after{box-shadow:inset 220px 0 260px rgba(0,183,235,.20)}
.cx-conn.dst-trakt::after{box-shadow:inset 220px 0 260px rgba(237,28,36,.18)}
.cx-conn .wm-dim{position:absolute;inset:0;pointer-events:none;z-index:1;border-radius:16px;background:radial-gradient(120% 140% at 10% 20%,rgba(0,0,0,.22),transparent 60%),radial-gradient(120% 140% at 90% 20%,rgba(0,0,0,.22),transparent 60%);opacity:.9}
.cx-conn>.split-border{display:none!important}

/* Drag & drop */
.pair-card[draggable=true]{cursor:grab}
.pair-card.dragging{opacity:.82;transform:scale(.985) rotate(.6deg);animation:cx-wiggle .35s ease-in-out infinite;z-index:10}
.drag-placeholder{border-radius:16px;outline:2px dashed rgba(255,255,255,.24);outline-offset:-2px;min-height:68px;margin:6px 0;background:rgba(255,255,255,.04)}
...
/* Delete animation */
.pair-card.removing{opacity:0;transform:translateY(-10px) scale(.98);height:0!important;margin:0!important;padding:0!important;overflow:hidden!important;transition:opacity .25s ease,transform .25s ease,height .25s ease,margin .25s ease,padding .25s ease}
