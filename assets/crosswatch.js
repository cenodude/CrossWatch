...
function setValIfExists(id, val){ const el=document.getElementById(id); if(el) el.value = val ?? ''; }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{ try{ scheduleInsights(); }catch(_){}});
...
function scheduleInsights(max=20){
  ...
  const tick = async ()=>{
    ...
    const hasTargets = document.getElementById('sync-history') || document.getElementById('watchtime') || document.getElementById('watchtime-note');

    if (hasTargets){

      try { await refreshInsights(); } catch(e){ console.warn('refreshInsights failed', e); }
...
async function refreshInsights(){
  ...
  try {
    ...
    if (hEl) {
      ...
      hEl.innerHTML = (hist.map(row => {
        ...
        return `<div class="history-item">
          ...
          <div class="history-meta">${when} • <span class="badge ${badgeClass}">${row.result||'—'}</span> • ${dur}s</div>
    ...
    const wt = data.watchtime || {hours:0, days:0, minutes:0, movies:0, shows:0, method:'fallback'};
    ...
    if (note) {
      ...
      note.textContent = (wt.method==='mixed') ? 'TMDb + SIMKL' :

                         (wt.method==='tmdb')  ? 'TMDb only' :

                         (wt.method==='simkl') ? 'SIMKL only' : 'estimate';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try { window._cfgCache = cfg; updateSimklButtonState(cfg); } catch(_) {}
  ...
  try { window._cfgCache = cfg; } catch(_) {}
...
// Sync Options
  ...
  try {
    ...
    try { notify && notify('Failed to request PIN'); } catch(_){}
  ...
  if (!data || data.ok === false) {
    ...
    try { notify && notify(data && data.error ? data.error : 'PIN request failed'); } catch(_){}
  ...
  try {
    ...
    try {
      ...
      document.querySelectorAll('#plex_pin, input[name="plex_pin"]').forEach(el => { try { el.value = pin; } catch(_){ } });
    ...
    if (pin) { try { await navigator.clipboard.writeText(pin); } catch(_) {} }
...
function updateSimklButtonState(cfgOpt){
  try {
    ...
    const cid = (document.getElementById('simkl_client_id')?.value || cfg.simkl?.client_id || '').trim();
    const sec = (document.getElementById('simkl_client_secret')?.value || cfg.simkl?.client_secret || '').trim();
...
if (typeof updateSimklHint !== 'function') { function updateSimklHint(){} }
...
function startPlexTokenPoll(){
  ...
  try { if (plexPoll) { clearInterval(plexPoll); } } catch(_) {}
  ...
  plexPoll = setInterval(async ()=>{
    ...
    if (tok){
      ...
      try { const el = document.getElementById('plex_token'); if (el) el.value = tok; } catch(_){}
...
async function loadProviders(){
  ...
  try{
    ...
    div.innerHTML = html;
    try { window.cx.providers = Array.isArray(arr) ? arr : []; } catch(e){ window.cx.providers = []; }
    try { if (typeof renderConnections === 'function') renderConnections(); } catch(e){ console.warn('renderConnections failed', e); }
  }catch(e){
...
function _cap(obj, key){ try { return !!(obj && obj.features && obj.features[key]); } catch(_){ return false; } }
function _byName(list, name){ name = String(name||'').toUpperCase(); return (list||[]).find(p => String(p.name||'').toUpperCase()===name); }
function _normWatchlistFeature(val){
  if (val && typeof val === 'object') return { add: !!val.add, remove: !!val.remove };
...
function renderConnections(){ cxEnsureStyles(); hideLegacyPairsUI();
  ...
  const cards = provs.map(p=>{
    ...
    return `<div class="prov-card" data-prov="${p.name}">
      ...
      <button class="btn neon" onclick=\"cxToggleConnect('${p.name}')\">Connect</button>
    </div>`;
  }).join('');

  // Build pairs board
  const pairCards = (pairs||[]).map(pr=>{
  ...
  if(!document.getElementById('cx-modal')){
    const modal = document.createElement('div');
    modal.id = 'cx-modal';
    modal.className = 'modal-backdrop hidden';
    modal.innerHTML = `<div class="modal-card">
      <div class="modal-header"><div class="title">Configure Connection</div><button class="btn-ghost" onclick="cxCloseModal()">✕</button></div>
      <div class="modal-body">
        <div class="grid2">
          <div><label>Source</label><input id="cx-src" type="text" readonly></div>
          <div><label>Target</label><input id="cx-dst" type="text" readonly></div>
        </div>
        <div class="grid2">
          ...
          <div><label>Enabled</label>
            <label class="switch"><input id="cx-enabled" type="checkbox" checked><span></span></label>
          </div>
        </div>
        ...
        <div class="grid2">
          <div>
            ...
            <div class="radio-row">
              <label><input type="checkbox" id="cx-wl-add" checked> Add</label>
              <label class="muted"><input type="checkbox" id="cx-wl-remove" disabled> Remove</label>
            </div>
      ...
      <div class="modal-footer">
        <button class="btn acc" id="cx-save">Save</button>
        <button class="btn" onclick="cxCloseModal()">Cancel</button>
      </div>
    </div>`;
...
/* === Modal control === */
function cxOpenModalFor(pair, editingId){
  // Compute capability intersection for watchlist
  ...
  document.getElementById('cx-wl-add').checked = wlOk && !!wf.add;
  document.getElementById('cx-wl-add').disabled = !wlOk;
  document.getElementById('cx-wl-remove').checked = false;
  document.getElementById('cx-wl-remove').disabled = true;
  document.getElementById('cx-wl-note').textContent = wlOk ? '' : 'Watchlist is not supported on one of the providers.';

  const saveBtn = document.getElementById('cx-save');
  saveBtn.onclick = async ()=>{
    const mode = (document.querySelector('input[name="cx-mode"]:checked')||{}).value || 'one-way';
    ...
    try{
      ...
      console.warn('Save connection failed', e);
...
async function cxToggleEnable(id, on){
  try{
    await fetch(`/api/pairs/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({enabled: !!on}) });
    await loadPairs();
  }catch(e){ console.warn('toggle enable failed', e); }
}
...
window.cxToggleConnect = function(name){
  ...
  constsel = window.cx.connect;
  if(!constsel.source){
    cxStartConnect(name);
    try { logToSyncOutput(`[ui] Source selected: ${name}. Now click a target.`); } catch(_){}
    return;
  }
  if(constsel.source && constsel.source !== name){
    cxPickTarget(name);
    return;
  }
  // same card clicked again -> cancel selection
  window.cx.connect = { source:null, target:null };
  try { logToSyncOutput(`[ui] Selection cleared.`); } catch(_){}
};
...
function cxEnsureStyles(){
  ...
  `;
  const style = document.createElement('style'); style.id='cx-style'; style.textContent = css; document.head.appendChild(style);
}
...
if (typeof window.loadPairs !== 'function') {
  async function loadPairs(){
    ...
    }catch(e){ console.warn('[fallback] loadPairs failed', e); }
  }
...
if (typeof window.deletePair !== 'function') {
  async function deletePair(id){
    if(!id){ console.warn('deletePair: missing id'); return; }
    try{
      await fetch(`/api/pairs/${id}`, { method:'DELETE' });
      try { await loadPairs(); } catch(_){}
    }catch(e){ console.warn('[fallback] deletePair failed', e); }
  }
...
function _hideLegacyPairsUIHard(){
  try{
    ...
    Array.from(document.querySelectorAll('button, .btn')).forEach(b => {
      ...
      if (t === 'add pair' || t === '+ add pair' || t === 'add batch' || t === '+ add batch') { b.style.display = 'none'; }
    });
    // Batch section containers — try several likely IDs/classes
    const guess = ['batches_list','batches','sec-batches','batch-list'];
    guess.forEach(sel => {
      const el = document.getElementById(sel) || document.querySelector('#'+sel) || document.querySelector('.'+sel);
...
/* Injected minimal layout fix for connections board */
(function(){
  if(document.getElementById('cx-align-style')) return;
