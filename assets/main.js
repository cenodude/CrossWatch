(() => {
  ...
  async function pullSummary() {
    ...
    if (!mapped.done && !running && (exitedOk || s?.finished || s?.end)) {
      mapped.done = true; mapped.start = true; mapped.pre = true; mapped.post = true;
    }

    timeline = mapped;
    progressPct = asPctFromTimeline(timeline);

    // bridge: legacy run-button & preview
    try {
      ...
      if (typeof startRunVisuals === "function" && typeof stopRunVisuals === "function") {
        if (running && !pullSummary._wasRunning) {
      ...
      if (pullSummary._wasRunning && !running) {
        try {
      ...
      pullSummary._wasRunning = running;
    } catch (e) {}

    if (!summary.enabled) {
      summary.enabled = defaultEnabledMap(); // ensure enabled present
    }
