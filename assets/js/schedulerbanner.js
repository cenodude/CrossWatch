...
/* State */
let S={enabled:false,running:false,next:0,busy:false}, W={enabled:false,alive:false,busy:false}, H={enabled:false,busy:false};
...
function renderWatch(){
  ...
  chip.style.display='inline-flex';
  chip.classList.toggle('ok',!!W.alive);
  chip.classList.toggle('bad',!W.alive);
...
function renderHook(){
  ...
  chip.classList.toggle('ok',true);
  chip.classList.toggle('bad',false);
  chip.classList.toggle('live',false);
  sub.textContent='Webhook: enabled';
}
...
/* Read scrobble once → derive both Watcher and Webhook states */
...
try{
  ...
  if(W.enabled){
    const s=await fetch('/debug/watch/status?t='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({}));
    W.alive=!!s?.alive;
  } else { W.alive=false }

  H.enabled=enabled && mode==='webhook';
}catch{ W.enabled=false; W.alive=false; H.enabled=false }
