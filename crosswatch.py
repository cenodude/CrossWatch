...
Backend for status, auth, scheduling, and sync (provider-agnostic via Orchestrator).
Remarks are in English only.
"""
...
from fastapi import Body, FastAPI, Query, Request, Path as FPath # type: ignore
...
class MetadataResolveIn(BaseModel):
    ...
    ids: dict                    # e.g. {"tmdb": 123, "imdb": "tt..."}
    locale: str | None = None
    need: dict | None = None     # e.g. {"poster": True}
...
# -----------------------------------------------------------------------------
# Config + app paths (via shared config module)
# -----------------------------------------------------------------------------
# CONFIG_DIR is the resolved config folder (e.g. /config in Docker, repo root locally)
...
# Prefer the config/state paths managed by the orchestrator; keep these for UI fallbacks:
...
def _normalize_features(f: dict | None) -> dict:
    ...
    for k in FEATURE_KEYS:
        ...
        if isinstance(v, bool):
            # True → enable+add; never default to remove=True
...
async def _lifespan(app):
    # 1) optional legacy startup hook
    try:
    ...
    try:
        ...
        # 3) optional legacy shutdown hook
        try:
...
def _orc_progress(event: str, data: dict):
    # Bridge Orchestrator progress events to UI log
    try:
...
def _feature_enabled(fmap: dict, name: str) -> tuple[bool, bool]:
    """Return (enabled, allow_removals) for a feature mapping."""
...
def _item_sig_key(v: dict) -> str:
    """Dedup key for wall: prefer IDs, else type+title+year."""
    ids = (v.get("ids") or {})
...
def _persist_state_via_orc(orc: Orchestrator, *, feature: str = "watchlist") -> dict:
    """Rebuild state.json from live snapshots (provider-agnostic)."""
    snaps = orc.build_snapshots(feature=feature)  # {'PROV': {key: item}}
...
def _run_pairs_thread(run_id: str, overrides: dict | None = None) -> None:
    ...
    # clear UI buffer and print start marker
    LOG_BUFFERS["SYNC"] = []
    ...
    try:
        ...
        try:
            # IMPORTANT: ensure we load the latest orchestrator from disk
        ...
        # Use compat 'progress=' callback only; our orchestrator will JSON-emit events into it.
        mgr = Orchestrator(config=cfg)

        result = mgr.run_pairs(
            dry_run=False,
            progress=_sync_progress_ui,     # <- single-arg logger; receives JSON-encoded events
...
# -----------------------------------------------------------------------------
# Progress parsing (kept for UI compatibility)
# -----------------------------------------------------------------------------
...
def refresh_wall():
    state = _load_state()
    _ = _load_hide_set()
    # your UI renderer uses /api/state/wall; keep this simple
...
def _clear_watchlist_hide() -> None:
    """Clear watchlist_hide.json after sync (atomic)."""
    try:
        p = HIDE_PATH
        if not p.exists():
            return
...
def _state_items(state: dict | None, provider: str) -> dict:
    """
    Read watchlist items for a provider from the new snapshot layout,
    with a legacy fallback.

    New:  providers.<PROVIDER>.watchlist.baseline.items
    Old:  providers.<PROVIDER>.items
    """
...
def _wall_items_from_state() -> List[Dict[str, Any]]:
    ...
    for pk, b in buckets.items():
        ...
        # legacy-friendly status (keeps current CSS): 2+ sources -> "both"
...
async def _watch_start():
    try:
        ...
        _UIHostLogger("TRAKT","WATCH")(f"watch failed: {e}", level="ERROR")
...
def debug_watch_status():
    w = WATCH # type Optional WatchService
    if w is None:
...
def debug_watch_start():
    w = _ensure_watch_started() # returns WatchService
    alive = bool(w._thr and w._thr.is_alive())
...
async def webhook_trakt(request: Request):
    ...
    def log(msg, level="INFO"):
        try: logger(msg, level=level, module="SCROBBLE")
        except: pass
...
# -----------------------------------------------------------------------------
# Metadata resolve (via MetadataManager)
# -----------------------------------------------------------------------------
...
def api_sync_providers() -> JSONResponse:
    ...
    items.sort(key=lambda x: x.get("label", x.get("name", "")))
...
def api_pairs_list() -> JSONResponse:
    try:
        cfg = load_config()
        arr = _cfg_pairs(cfg)
        # migrate/normalize features on the fly
        dirty = False
        for it in arr:
...
def api_pairs_reorder(order: List[str] = Body(...)) -> dict:
    ...
    try:
        ...
        return {"ok": True, "reordered": changed, "count": len(new_arr), "unknown_ids": unknown_ids,
                "final_order": [str(p.get("id")) for p in new_arr]}
    except Exception as e:
        try: _append_log("TRBL", f"/api/pairs/reorder failed: {e}")
        except: pass
        return {"ok": False, "error": str(e)}
...
def api_trbl_reset_state(
    ...
    try:
        ...
        if mode in ("clear_tombstones", "clear_both"):
            try: tomb_path.unlink(missing_ok=True)
            except Exception: pass
